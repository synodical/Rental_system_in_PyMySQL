...
def customer_insert_model(cur_cid) :
    ...
    BID = column_values[0]
    mno = column_values[1]
    rentaldate = column_values[2] # 입력

    sql = "select type, rentaldate from model where mno =" + "'" + mno + "'"
    cursor.execute(sql)
    conn.commit()
    ...
    w_file.write("> " + BID + " " + mno + " " + mtype + " " + rentaldate + "\n")
...
def customer_cancel(cur_cid) :
    ...
    sql = "update model set rentaldate = null, cid = null" + \
